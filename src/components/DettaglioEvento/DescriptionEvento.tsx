...
const DescriptionEvento = (props: HeroDettaglioProps) => {
    ...
    const {tags, description, price, includedDrinks, dresscode, includedDishes} = props.eventDetail
        ...
        <>
            <section className='descrizione-evento my-10'>
                <div className="container">
                    <div className="flex">
                        <div className="descrizione-evento__testo-grande w-3/6 my-10">
                            ...
                            {description.long.map((frase, i)=> {
                                if(i < 1) {
                                    return (<p key={i} className='pb-5'>{frase}</p>)
                                }
                            })}
                        </div>
                        <div className="descrizione-evento__testo-piccolo w-3/6 my-10 ml-10">
                            ...
                            {description.long.map((frase, i)=> {
                                if(i >= 1) {
                                    return (<p key={i} className='pb-3'>{frase}</p>)
                                }
                            })}
                        </div>
                    </div>
                    ...
                    <div className="flex">
                        <div className="descrizione-evento__testo-grande w-3/6 my-10">
                            Fame? Abbiamo anche pensato al cibo 🍕
                            Nel prezzo sono inclusi tutti questi deliziosi piatti per insaporire la serata!
                        </div>
                        <div className="descrizione-evento__testo-piccolo w-3/6 my-10 ml-10">
                            {/* La descrizione è un array di frasi: con il map, ogni frase diventa un p e così ogni paragrafo va a capo */}
                            {includedDishes.map((dish, i)=> {
                                return (
                                    <>
                                        <p key={i} className='pb-3'>Piatto: {dish.name}</p>
                                        <p key={i} className='pb-3'>Descrizione: {dish.description}</p>
                                        
                                        {dish.allergens.map((ingrediente, i)=> {
                                            return (
                                                <>
                                                    <p key={i} className='pb-3'>Allergeni: {ingrediente}</p>
                                                </>
                                            )
                                        })}
