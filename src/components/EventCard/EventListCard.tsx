...
const EventListCard = ({ singleEvent, detailPath }: EventListCardPropsType) => {
    ...
    const { name, date, price, tags, coverImage, description } = singleEvent;

    // formattazione della data > es. lunedi 25/12
    const dateFormatted = new Date(date).toLocaleString("it-IT", {
        weekday: "long",
        month: "2-digit",
        day: "2-digit",
    });
    ...
    return (
        <Link to={detailPath}>
            <div
                className="event-card h-[417px] flex flex-col justify-between"
                style={{
                    backgroundImage: `linear-gradient(rgba(206, 75, 197, 0.4), rgba(91, 29, 191, 0.5), rgba(0, 0, 0, 0.5)), url(${coverImage})`,
                }}
            >
                ...
                <div className="event-card__bottom-info">
                    ...
                    <div className="flex">
                        <div className="hour">
                            ⏰ dalle {hourFormatted}:
                            {minFormatted.toString().length < 2 ? minFormatted + "0" : minFormatted}
                        </div>
